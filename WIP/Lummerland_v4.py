#!/usr/bin/env python3
# =============================================================
#  🏝️  LUMMERLAND ISLAND ENERGY MODEL v4.0  – Lummerland + Lummerstadt (Greifswald)
#  Reines Python-Skript – kein Jupyter / Colab erforderlich
#  Ausführen: python Lummerland_v4.py
#
...
# =============================================================
#  0) ABHÄNGIGKEITEN AUTO-INSTALLIEREN (vor den Imports!)
# =============================================================
...
# =============================================================
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Lummerland_v4_Output")
...
print("=" * 55)
print(" LUMMERLAND v4.0 – Lummerland + Lummerstadt (Greifswald)")
...
def _shift_profile(
    profile: np.ndarray, shift: int = 0, scale: float = 1.0,
    noise: float = 0.0, seed: int = 0,
) -> np.ndarray:
    """Leitet aus einem Profil ein verwandtes ab (Lummerstadt):
    zeitversetzt um `shift` Stunden, skaliert und leicht verrauscht."""
...
def _get_gurobi_env() -> "gp.Env":
    """Erzeugt die Gurobi-WLS-Umgebung genau einmal und verwendet sie wieder
    (spart pro Lauf einen Lizenz-Checkout über das Netzwerk)."""
    global _gurobi_env
...
def solve_network(net: "pypsa.Network", verbose: bool = True) -> bool:
    """Löst das Netz mit Gurobi (falls lizensiert), sonst HiGHS.
    Lösung & Shadow-Prices werden von PyPSA automatisch zugewiesen.
    Gibt True zurück, wenn Gurobi verwendet wurde."""
    if USE_GUROBI:
        try:
    ...
    net.optimize(solver_name="highs",
                 solver_options={"time_limit": 600, "simplex_scale_strategy": 2})
    return False

def total_co2(net: "pypsa.Network") -> float:
    """Gesamte CO₂-Emissionen des gelösten Netzes (Konvention wie bisher: /1e3)."""
    try:
        return float(
            (net.generators_t.p * net.generators.carrier.map(net.carriers.co2_emissions))
            .sum().sum() / 1e3
        )
...
try:
    GUROBI_USED = solve_network(n, verbose=True)
except Exception as _e:
    print(f"⚠ Solver: {_e} → 2h-Fallback")
    n.set_snapshots(n.snapshots[::2])
    GUROBI_USED = solve_network(n, verbose=True)
print("✓ Gurobi erfolgreich" if GUROBI_USED else "✓ HiGHS erfolgreich")
...
fig_a.suptitle("LUMMERLAND v4.0 – Erzeugungsmix, H₂-System & Sektorkopplung",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_a = os.path.join(OUTPUT_DIR, "lummerland_v4_ergebnisse.png")
...
fig_b.suptitle("LUMMERLAND v4.0 – Kosten, CO₂ & Monte-Carlo Robustheit",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_b = os.path.join(OUTPUT_DIR, "lummerland_v4_kosten.png")
...
_p_m  = n.generators_t.p.copy()
_p_m.index = n.snapshots
monthly = (_p_m.groupby(_p_m.index.month).sum() * _step_h / 1e3).reindex(range(1, 13), fill_value=0.)
bot2  = np.zeros(12)
for g in n.generators.index:
    vals = monthly[g].values
...
fig_c.suptitle("LUMMERLAND v4.0 – Sektorkopplung, Speicher & Versorgungssicherheit  |  inkl. Lummerstadt",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_c = os.path.join(OUTPUT_DIR, "lummerland_v4_sektoren.png")
...
ax_map.set_title(
    f"LUMMERLAND v4.0 – Lummerland + Lummerstadt (Greifswald)  |  {'✓ Echte Daten' if DATA_OK else '⚠ Demo-Fallback'}",
    fontsize=14, fontweight="bold", color="white", pad=15,
)
fig_map.tight_layout()
_path_map = os.path.join(OUTPUT_DIR, "lummerland_v4_karte.png")
...
pdf_path = os.path.join(OUTPUT_DIR, "Lummerland_v4_Bericht.pdf")
...
with PdfPages(pdf_path) as pdf:
    ...
    ac.text(0.5, 0.60, "Island Energy Model v4.0 – Lummerland + Lummerstadt", ha="center", fontsize=24, color="white", transform=ac.transAxes)
    ...
    d = pdf.infodict()
    d["Title"]        = "Lummerland Island Energy Model v4.0"
