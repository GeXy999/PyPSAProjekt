#!/usr/bin/env python3
# =============================================================
#  🏝️  LUMMERLAND ISLAND ENERGY MODEL v4.1  – Lummerland + Lummerstadt (Greifswald)
#  Reines Python-Skript – kein Jupyter / Colab erforderlich
#  Ausführen: python Lummerland_v4_1.py
#
...
#  ✓ Code aufgeraeumt: zentrale Konfiguration, weniger Duplikate, robustere Solver-Logik
# =============================================================
...
# =============================================================
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Lummerland_v4_1_Output")
...
print("=" * 55)
print(" LUMMERLAND v4.1 – Lummerland + Lummerstadt (Greifswald)")
...
GUROBI_USED = False
if GUROBI_AVAILABLE and GRB_WLSACCESSID and GRB_WLSSECRET and GRB_LICENSEID is not None:
    try:
        env = gp.Env(params={
...
for mc_i in range(N_MC):
    ...
    try:
        ...
        solver_mc = solve_network(nm, prefer_gurobi=True, output_flag=0, highs_time_limit=MC_SOLVER_TIME)
                _mc_solved = True
...
ls = n.loads_t.p_set[[c for c in POWER_LOADS if c != "Last_Lstadt"]].iloc[idx_summer].sum(axis=1)
...
lw2 = n.loads_t.p_set[[c for c in POWER_LOADS if c != "Last_Lstadt"]].iloc[idx_winter].sum(axis=1)
...
fig_a.suptitle("LUMMERLAND v4.1 – Erzeugungsmix, H₂-System & Sektorkopplung",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_a = os.path.join(OUTPUT_DIR, "lummerland_v4_1_ergebnisse.png")
...
fig_b.suptitle("LUMMERLAND v4.1 – Kosten, CO₂ & Monte-Carlo Robustheit",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_b = os.path.join(OUTPUT_DIR, "lummerland_v4_1_kosten.png")
...
fig_c.suptitle("LUMMERLAND v4.1 – Sektorkopplung, Speicher & Versorgungssicherheit  |  inkl. Lummerstadt",
               fontsize=14, fontweight="bold", color="white", y=0.97)
_path_c = os.path.join(OUTPUT_DIR, "lummerland_v4_1_sektoren.png")
...
ax_map.set_title(
    f"LUMMERLAND v4.1 – Lummerland + Lummerstadt (Greifswald)  |  {'✓ Echte Daten' if DATA_OK else '⚠ Demo-Fallback'}",
    fontsize=14, fontweight="bold", color="white", pad=15,
)
fig_map.tight_layout()
_path_map = os.path.join(OUTPUT_DIR, "lummerland_v4_1_karte.png")
...
pdf_path = os.path.join(OUTPUT_DIR, "Lummerland_v4_1_Bericht.pdf")
...
with PdfPages(pdf_path) as pdf:
    ...
    ac.text(0.5, 0.60, "Island Energy Model v4.1 – Lummerland + Lummerstadt", ha="center", fontsize=24, color="white", transform=ac.transAxes)
    ...
    d = pdf.infodict()
    d["Title"]        = "Lummerland Island Energy Model v4.1"
